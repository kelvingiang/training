...
//        $this->avg();
    ...
    public function create() {
        ...
        $file_ip = fopen(DIR_FILE . 'counter/ip.txt', 'rb');
        while (!feof($file_ip)) {
            $line[] = fgets($file_ip);
        }
        ...
        if (!($found == 1)) {
            $file_ip2 = fopen(DIR_FILE . 'counter/ip.txt', 'ab');
            $line = "$ip\n";
            fwrite($file_ip2, $line, strlen($line));
            $file_count = fopen(DIR_FILE . 'counter/count.txt', 'rb');
            $data = '';
            ...
            $file_count2 = fopen(DIR_FILE . 'counter/count.txt', 'wb');
    ...
    public function online() {
        ...
        $file1 = DIR_FILE . "counter/online.log";
        $lines = file($file1);
        $line2 = "";
    ...
    public function avg() {
        $file_count = fopen(DIR_CLASS . 'counter/count.txt', 'rb');
        $data = '';
        while (!feof($file_count))
            $data .= fread($file_count, 4096);
        fclose($file_count);
